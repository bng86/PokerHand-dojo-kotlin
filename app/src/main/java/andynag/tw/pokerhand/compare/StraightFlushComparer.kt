...
import andynag.tw.pokerhand.Player
import andynag.tw.pokerhand.Winner

class StraightFlushComparer(private val player1: Player, private val player2: Player) :
    HandsCategoryComparer {
    override fun getWinner(): Winner {
        ...
        return when {
            player1KeyCard > player2KeyCard -> {
                Winner.KeyCard(
                    player1.name,
                    player1.hands.getHandCategory(),
                    player1KeyCard.originalPoint()
                )
            }
            player1KeyCard < player2KeyCard -> {
                Winner.KeyCard(
                    player2.name,
                    player2.hands.getHandCategory(),
                    player2KeyCard.originalPoint()
                )
            }
